...
class RunManager {
  constructor({ name, token, client, logger, rotationMs }) {
    this.name = name;
    this.token = token;
    this.client = client;
    this.logger = logger;
    this.rotationMs = rotationMs;
    this.session = new FreeSession({ name, token, client, logger });
    this.runs = new Map();
    this.draining = [];
  ...
  async rotateAgent(agentID) {
    ...
    this.runs.set(agentID, { id: runId, agentID, startedAt: Date.now(), inflight: 0, requestCount: 0, finishing: false });
    this.lastError = '';
    if (oldRun) {
  ...
  async maintain() {
    try {
      ...
      this.logger.logErr(`${this.name}: refresh free session failed: ${e.message}`);
    }
    const toRotate = [];

...
// The CLI mints client_id per request as Math.random().toString(36).substring(2, 15)
// (sdk/src/run.ts promptId): a fresh 13-char base36 id. The server's free-mode
// gate validates this exact shape - a UUID-shaped client_id reads as a scripted
// caller (trefeon/freebuff-proxy's troubleshooting calls the format out explicitly).
function clientBase36Id() {
  let id = Math.random().toString(36).slice(2, 15);
  while (id.length < 13) id += Math.floor(Math.random() * 36).toString(36);
  return id.slice(0, 13);
}

// Billing modes the upstream accepts (see create-run-config.ts costMode union).
// A typo here would silently send an invalid value upstream and misbill, so
// unknown values fall back to 'free'.
const COST_MODES = new Set(['free', 'lite', 'normal', 'max', 'experimental', 'ask']);

// Read lazily (per request) so a --cost-mode CLI arg, which is applied after
// module load, still takes effect.
function currentCostMode() {
  const mode = (process.env.FB_COST_MODE || 'free').trim().toLowerCase();
  return COST_MODES.has(mode) ? mode : 'free';
}
...
function injectMetadata(payload, { model, runId, sessionInstanceID, traceSessionId }) {
  ...
  // Fresh 13-char base36 id per request - the exact shape the CLI mints
  // (Math.random().toString(36).substring(2, 15)); the gate validates it.
  metadata.client_id = clientBase36Id();
  // The CLI sends trace_session_id (a UUID) in the metadata, minted ONCE per
  // run and reused for every request in that run (sdk/src/run.ts:279-280 -
  // previousRun?.traceSessionId ?? randomUUID). The proxy mints it when the
  // run is created and threads it through; fall back to a fresh UUID here.
  metadata.trace_session_id = traceSessionId || crypto.randomUUID();
  // Mirrors the CLI: Freebuff's LITE agent mode sends cost_mode 'free', which
  // is the server's switch for "0 credits on allowlisted agent+model combos".
  // Without it the request is billed as paid and a credit-less account gets
  // HTTP 402 Out of credits (the CLI defaults to 'normal' when absent).
  metadata.cost_mode = currentCostMode();
  if (sessionInstanceID) metadata.freebuff_instance_id = sessionInstanceID;
  cloned.codebuff_metadata = metadata;
  // Routing options, not a data-collection opt-out: the real CLI sends
  // { order: providerOrder[model], allow_fallbacks: !isExplicitlyDefined }
  // (sdk/src/impl/llm.ts:95-106) and for models outside the statically
  // defined set (like ours) order is undefined, leaving only
  // allow_fallbacks:true. The earlier { data_collection: 'deny' } was ported
  // from trefeon/freebuff-proxy, which itself credits proxy-freebuff - it is
  // not a real CLI field and never passed the gate.
  cloned.provider = { allow_fallbacks: true };
  // The CLI does NOT send a stop list on plain chat (no stopSequences wired
  // in cli/src); cb_easp is an internal agent step param, not a model stop
  // sequence. Leave whatever the caller supplied untouched.
  // The official CLI ALWAYS streams upstream (the free-mode gate rejects
  // sync chat calls as "calling the API directly"). The reference forces
  // stream:true on every chat request; sync clients still get a JSON
  // completion back because server.js accumulates the SSE and converts.
  cloned.stream = true;
  return JSON.stringify(cloned);
}

// Accumulates an upstream SSE chat stream (the upstream is always asked to
// stream) into a single non-streaming chat.completion JSON body, port of
// trefeon/freebuff-proxy's Accumulator. Used to serve sync clients. Accepts
// both "data: {...}" SSE lines and bare JSON lines (the reference's parser
// does the same), and caps total buffered bytes like the sync readBody did.
function accumulateOpenAIStream(upstream, limit = 64 * 1024 * 1024) {
  return new Promise((resolve, reject) => {
    ...
    function onData(c) {
      ...
      if (size > limit) {
        cleanup();
        upstream.destroy();
        reject(new Error('upstream stream too large'));
        return;
      }
      buffer += c.toString();
      let idx;
    ...
    const onEnd = settle(() => {
      ...
      cleanup();
      resolve(JSON.stringify(buildCompletion(acc)));
    });
    const onError = settle((e) => {
      cleanup();
      reject(e);
    });
...
function accumulateChunk(acc, chunk) {
  if (sval(chunk.id).trim()) acc.id = chunk.id;
  const created = numInt(chunk.created);
  if (created > 0) acc.created = created;
  if (sval(chunk.model).trim()) acc.model = chunk.model;
  if (chunk.usage != null) acc.usage = chunk.usage;
  if (sval(chunk.system_fingerprint).trim()) acc.systemFingerprint = chunk.system_fingerprint;
  const choice = chunk.choices && chunk.choices[0];
  if (!choice) return;
  const delta = choice.delta || {};
  if (typeof delta.content === 'string' && delta.content) acc.contentParts.push(delta.content);
  if (typeof delta.reasoning_content === 'string' && delta.reasoning_content) acc.reasoningParts.push(delta.reasoning_content);
  for (const tc of delta.tool_calls || []) {
    const index = numInt(tc.index) ?? 0;
    let cur = acc.toolCalls.get(index);
    if (!cur) {
      cur = { index, id: 'call_' + clientSessionId(), type: 'function', function: { name: '', arguments: '' } };
      acc.toolCalls.set(index, cur);
    }
    if (sval(tc.id).trim()) cur.id = tc.id;
    if (tc.function) {
      if (sval(tc.function.name).trim()) cur.function.name = tc.function.name;
      if (typeof tc.function.arguments === 'string') cur.function.arguments += tc.function.arguments;
    }
  }
  if (sval(choice.finish_reason).trim()) acc.finishReason = choice.finish_reason;
}

function buildCompletion(acc) {
  const message = { role: 'assistant', content: acc.contentParts.join('') };
  if (acc.reasoningParts.length) message.reasoning_content = acc.reasoningParts.join('');
  if (acc.toolCalls.size) {
    message.tool_calls = [...acc.toolCalls.values()]
      .sort((a, b) => a.index - b.index)
      .map(({ index, ...tc }) => tc);
  }
  const response = {
    id: acc.id || 'chatcmpl-' + clientSessionId(),
  ...
  if (acc.systemFingerprint) response.system_fingerprint = acc.systemFingerprint;
  return response;
}

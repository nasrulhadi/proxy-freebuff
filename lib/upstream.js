...
class UpstreamClient {
  ...
  request(method, pathname, { token, body, timeoutMs, headers } = {}) {
    const url = new URL(pathname.replace(/^\//, ''), this.base);
    return this.requestLoop(url, method, { token, body, timeoutMs, headers }, 0);
  }

  requestLoop(url, method, { token, body, timeoutMs, headers }, hops) {
    return new Promise((resolve, reject) => {
      const req = https.request(
        ...
        (res) => {
          if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location && hops < 5) {
            ...
            resolve(this.requestLoop(next, follow ? method : 'GET', { token, body: follow ? body : null, timeoutMs, headers }, hops + 1));
            return;
          }
          resolve(res);
        }
  ...
  chatCompletions(token, body, { model, instanceId } = {}) {
